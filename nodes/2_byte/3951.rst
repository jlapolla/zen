...
things that client code could easily do itself. And, in fact, my other feature
...
Well, it's impossible to test all unsupported conditions, because there are an
infinite number of unsupported conditions. For example, we don't test the
behavior of ``sqrt()`` when the meanings of the standard Python math functions
have been altered. We also don't test the behavior of ``sqrt()`` when the system
is about to run out of memory. There are lots of unsupported conditions we don't
test; the list goes on forever.

Very well, we cannot test all unsupported conditions. But does that mean we are
prohibited from testing this particular unsupported condition? Is it wrong to
test ``sqrt(-1)``? Well, when we decide to test a feature, what does that imply
about the feature?

You ponder this for much longer than a moment. After much deliberation, you come
to this conclusion: when you test a feature, you are making the claim that the
feature is an officially supported feature. This means that you must maintain
the feature for backwards compatibility. In fact, this means that client code
can **RELY** on the feature being supported for the forseeable future. Testing
``sqrt(-1)`` means that client code is explicitly allowed to call ``sqrt(-1)``
just like the test does, and client code can rely on it raising a "domain error"
exception just like the test specifies.
