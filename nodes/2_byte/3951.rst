...
``checked_sqrt()``, you have already verified that ``x`` is an integer. Also,
because of the condition in the while loop, you know that ``x`` is non-negative
when you call ``checked_sqrt()``.

However, you know that ``checked_sqrt()`` is going to perform these checks
**again**, and you want your code to be fast. So, you skip the extra
``checked_sqrt()`` checks entirely, and just call ``sqrt()`` directly.
...
the checked version ``checked_topo_order(g)`` that checks ``graph.has_cycles()``
